...
def score_vulnerabilitiy(vuln: dict, host: dict, discovery_cost: int) -> int:
...
def score_result(result, scenario: dict) -> int:
    ...
    for host in scenario.get("hosts", []):
        ...
        for vuln in scenario.get("hosts", []):
            host_id = host["id"]
            discovered = result.discovered_vulns.get(host_id, set())

            for vuln in host.get("vulnerabilities", []):
                if vuln["id"] in discovered:
                    total += score_vulnerabilitiy(vuln, host, result.total_cost)

        compromised_bonus = 25 * len(result.compromised_hosts)

        return total + compromised_bonus